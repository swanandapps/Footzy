  ...
  useEffect(() => {
    const fetchPLData = async () => {
      const response = await axios(
        "https://api.football-data.org/v2/competitions/2021/standings?code=PL",
        {
          headers: {
    ...
    console.log("Use effect - getiing standings for all leagues");
  }, []);
  ...
  return (
    <div className="">
      <TabPanel value={value} index={0} dir="rtl">
        <div>
          <h4 style={{ "text-align": "left", "margin-left": "3%" }}>
            Premier League
          </h4>
